...
def parse_sqlite():
    # check if input/sqlite exists 
    if not os.path.exists("input/sqlite"):
        os.makedirs("input/sqlite")
        
    # go into databases/sqlite/test folder and get every .test file
    
    files = os.listdir("databases/sqlite/test")
    
    for file in files:
        if file.endswith(".test"):
            print(f"[-] Parsing {file} ...", end="")
            with open(f"databases/sqlite/test/{file}", "r") as f:
                lines = f.readlines()
                # create a new file in input/sqlite with the same name
                with open(f"input/sqlite/{file}", "w") as f2:
                    for line in lines:
                        f2.write(line)
